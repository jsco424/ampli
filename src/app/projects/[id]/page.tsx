...
import {
  ...
  Target, Users, Sparkles, RotateCcw, FileText,
...
export default function ProjectViewPage() {
  ...
  const handleRegenerate = async () => {
    ...
    try {
      ...
      await supabase.from('projects').update({ narrative, insights, charts, prompt: regenPrompt }).eq('id', id)
  ...
  const renderChart = (chart: any, i: number) => {
    ...
    return (
      <div key={i} className={`p-5 rounded-2xl border ${card}`}>
        ...
        <p className={`text-xs mb-4 ${dark ? 'text-zinc-400' : 'text-zinc-500'}`}>{chart.description}</p>
        <ResponsiveContainer width="100%" height={200}>
          {chart.type === 'bar' ? (
            <BarChart {...chartProps}>
              ...
              <Tooltip contentStyle={{ background: dark ? '#18181b' : '#fff', border: 'none', borderRadius: 8 }} />
    ...
    <div className={`min-h-screen ${base}`}>
      ...
      <main className="pt-20 px-6 max-w-5xl mx-auto pb-20">
        ...
        {(project.target_company || project.target_audience) && (
          <div className={`flex items-center gap-3 px-4 py-3 rounded-xl border mb-4 ${dark ? 'bg-zinc-900 border-zinc-800' : 'bg-zinc-50 border-zinc-200'}`}>
            {project.target_company && (
              <div className="flex items-center gap-1.5">
                <Target size={13} className="text-emerald-500" />
                <span className={`text-xs font-medium ${dark ? 'text-emerald-400' : 'text-emerald-600'}`}>
                  {project.target_company}
                </span>
              </div>
            )}
        ...
        {/* Regenerate Panel */}
        {showRegenerate && (
          <div className={`p-4 rounded-2xl border mb-4 ${dark ? 'bg-zinc-900 border-zinc-800' : 'bg-zinc-50 border-zinc-200'}`}>
            ...
            <div className="flex gap-2">
              <button onClick={() => { setShowRegenerate(false); setRegenPrompt('') }}
              ...
              <button onClick={handleRegenerate} disabled={!regenPrompt.trim() || regenerating}
                ...
                {regenerating
                  ? <><div className="w-3 h-3 border-2 border-white/30 border-t-white rounded-full animate-spin" /> Regenerating...</>
        ...
        {/* Tabs */}
        <div className={`flex gap-2 mb-6 p-1 rounded-2xl w-fit ${dark ? 'bg-zinc-900' : 'bg-zinc-100'}`}>
        ...
        {tab === 'narrative' && (
          <div className="space-y-6">
                ...
                {project.insights.map((insight: any, i: number) => (
                  <div key={i} className={`p-4 rounded-2xl border ${card}`}>
                    <div className="flex items-center justify-between mb-1">
                      <span className={`text-xs ${dark ? 'text-zinc-400' : 'text-zinc-500'}`}>{insight.title}</span>
                      {insight.trend === 'up' ? <TrendingUp size={14} className="text-emerald-500" /> :
                       insight.trend === 'down' ? <TrendingDown size={14} className="text-red-400" /> :
                       <Minus size={14} className="text-zinc-400" />}
                    </div>
                    <div className="text-xl font-bold mb-1">{insight.value}</div>
                    <p className={`text-xs ${dark ? 'text-zinc-500' : 'text-zinc-400'}`}>{insight.description}</p>
                  </div>
                ))}
              </div>
            )}
            <div className={`p-6 rounded-2xl border ${card}`}>
              ...
              <div className={`text-sm leading-relaxed whitespace-pre-wrap ${dark ? 'text-zinc-300' : 'text-zinc-600'}`}>
                {project.narrative}
              </div>
            </div>
          </div>
        ...
        {tab === 'data' && (
          <div className={`rounded-2xl border overflow-auto ${card}`}>
            ...
            <pre className={`p-4 text-xs overflow-auto max-h-96 ${dark ? 'text-zinc-300' : 'text-zinc-600'}`}>
              {project.raw_data}
            </pre>
          </div>
        )}
        ...
        {tab === 'notes' && (
          <div className={`rounded-2xl border ${card}`}>
            <div className="p-4 border-b flex items-center justify-between">
              ...
              <div className={`flex items-center gap-1.5 text-xs transition-opacity ${notesSaved ? 'opacity-100' : 'opacity-0'}`}>
            ...
            <div className="p-4">
              <p className={`text-xs mb-3 ${dark ? 'text-zinc-500' : 'text-zinc-400'}`}>
                Log pitch notes, feedback, next steps, and follow-up context. Auto-saves as you type.
              </p>
              <textarea
                value={notes}
