...
const PILLARS = [
  ...
  },
  {
    ...
    description:
      'Prompting, re-prompting to fix the framing, then manually building slides by hand realistically takes hours per deck. ampli runs the full pipeline — verified analysis to a branded export — in under a minute.',
  },
  {
    icon: Lock,
    accent: 'emerald',
    title: "Your client's data stays yours",
    description:
      "Pasting a client's campaign data into a consumer chat tool comes with no clear guarantee about how it's used or retained. ampli is built to handle client data professionally, not as a side effect of a general-purpose chat window.",
  },
  {
    icon: BarChart2,
    accent: 'red',
    ...
    description:
      "See how your numbers compare to real, pooled, anonymized results across your industry — a dataset that simply doesn't exist anywhere outside ampli's own Crowd Insights pool, no matter how good the prompt is.",
  },
  {
    icon: Layers,
    accent: 'purple',
    ...
    description:
      'Cleaning data in one tool, writing in another, building slides in a third means the final result has no single coherent voice. ampli keeps analysis, brand, and export in one connected place, every time, not reassembled by hand each session.',
  },
]
...
const COMPARISON_ROWS = [
  {
    label: 'Time per deck',
    diy: '3-4 hours of prompting and manual slide building',
    ampli: 'Under a minute, start to finish',
  },
  {
    label: 'Number accuracy',
    diy: 'The model does the math — can be subtly, confidently wrong',
    ampli: 'Every figure formula-verified against your raw data',
  },
  {
    label: 'Client data handling',
    diy: 'No clear guarantee on how uploaded data is used or retained',
    ampli: 'Built for professional client data, not a side effect of a chat window',
  },
  {
    label: 'Industry benchmarking',
    diy: "Not possible — this comparison data doesn't exist for you to prompt with",
    ampli: 'Real, pooled, anonymized benchmarks included automatically',
  },
  {
    label: 'Brand consistency',
    diy: 'Re-explain colors, tone, and logo placement every session',
    ampli: 'Applied automatically to every export, every time',
  },
  {
    label: 'Tooling',
    diy: 'Cleanup, writing, and slides split across separate disconnected tools',
...
export default function AboutPage() {
  ...
  return (
    <div className={`min-h-screen ${base}`}>
      <Navbar />
      <main className="pt-24 px-6 max-w-5xl mx-auto pb-24">
        {/* Hero */}
        <div className="text-center mb-16">
          ...
          <h1 className="text-4xl sm:text-5xl font-black tracking-tight leading-[1.1] mb-5">
            "Why wouldn't I just do this with <span className="text-blue-500">ChatGPT</span> or{' '}
            <span className="text-blue-500">Claude</span>?"
          </h1>
          <p className={`text-lg max-w-2xl mx-auto leading-relaxed ${muted}`}>
            A fair question. Here's the honest answer — not because AI is somehow different inside
            ampli, but because of everything built around it.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-16">
          {PILLARS.map((p, i) => {
            ...
            const isWide = i === 3 // Crowd Insights benchmarking — the headline differentiator, given a wider card
            return (
              <div
                key={i}
        ...
        <div className="mb-16">
          ...
          <div className={`rounded-2xl border overflow-hidden ${card}`}>
            ...
            >
              ...
              <div className="p-4">
                <p
                  className={`text-xs font-semibold uppercase tracking-wide flex items-center gap-1.5 ${dark ? 'text-blue-400' : 'text-blue-600'}`}
                >
                  With ampli
                </p>
              </div>
